...
public class CloudSpeechAPIREST extends Operator {
	...
	public List<ParameterType> getParameterTypes() {
		ParameterTypeString key =
				new ParameterTypeString(API_KEY, "Google Cloud Platform API key.", false, false);

		String[] types = { "local", "remote" };
		ParameterTypeCategory requestType =
				new ParameterTypeCategory(REQUEST_TYPE, "Whether to transcribe a local file or a file stored on GCS.", types, 0);
		requestType.setOptional(false);
		requestType.setExpert(false);

		String[] audioFileExtensions = { "mp3", "wav", "flac" };
		ParameterTypeFile audioFile =
				new ParameterTypeFile(PATH_TO_AUDIO_FILE, "Audio file to process.", true, audioFileExtensions);
		audioFile.setExpert(false);
		int[] index = { 0 };
		audioFile.registerDependencyCondition(
				new EqualTypeCondition(this, requestType.getKey(), types, false, index)
				);

		ParameterTypeString remoteURL =
				new ParameterTypeString(REMOTE_URI, "URI that points to a file which contains audio. Only Google Cloud Storage URIs are supported.", true, false);
		...
		ParameterTypeCategory encoding =
				new ParameterTypeCategory("encoding", "Specifies the encoding scheme of the supplied audio (of type AudioEncoding). If you have a choice in codec, prefer a lossless encoding such as FLAC or LINEAR16 for best performance.", encodings, 0);
		encoding.setOptional(false);
		encoding.setExpert(false);

		ParameterTypeInt sampleRateHertz =
				new ParameterTypeInt(SAMPLE_RATE, "Specifies the sample rate in Hz of the supplied audio. The sampleRateHertz field is optional for FLAC and WAV files where the sample rate is included in the file header.", 0, Integer.MAX_VALUE);

		String[] languages = { "en-US", "fr-FR", "de-DE", "es-ES" };
		ParameterTypeStringCategory languageCode =
				new ParameterTypeStringCategory(LANGUAGE, "Contains the language and region/locale to use for speech recognition of the supplied audio. The language code must be a BCP-47 identifier. Note that language codes typically consist of primary language tags and secondary region subtags to indicate dialects.", languages);

		ParameterTypeBoolean profanityFilter =
				new ParameterTypeBoolean(PROFANITY_FILTER, "Indicates whether to filter out profane words or phrases. Words filtered out will contain their first letter and asterisks for the remaining characters (e.g. f***). The profanity filter operates on single words, it does not detect abusive or offensive speech that is a phrase or a combination of words.", false, false);
	...
	private JSONObject queryOperation(String operationName, String key) throws IOException, InterruptedException {
		HttpURLConnection connection =
				(HttpURLConnection) new URL("https://speech.googleapis.com/v1/operations/" + operationName + "?key=" + key).openConnection();
	...
	public void doWork() throws OperatorException {
		...
		configJSON = new JSONObject()
				...
				.put("profanityFilter", profanityFilter);
		...
		if (this.getParameterAsString("request type").equals("local")) {
			try {
				...
				audioJSON = new JSONObject()
						.put("content", audioBytes.toStringUtf8());
